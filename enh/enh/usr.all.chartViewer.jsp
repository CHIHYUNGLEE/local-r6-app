...
//가동율에 따른 실제 수행일
ProjectYearViewer.prototype.realDays = function(value){
	this.real += value;
}
...
ProjectYearViewer.prototype.performPeriod = function(sdate,edate,percent){
...
ProjectYearViewer.prototype.addData = function(obj){
	...
	overPerform =function(event) { 
		...
		//가동율에 따른 실제 수행일(calReal) 적용
		$(toolTip).find('#TOOLTIP_DATE')
			.html( calReal +' / ' + yearDays +' * 100 ') ;
	...
	this.performPeriod(obj.sDate,obj.eDate,obj.percent);
	...
	if(this.dprtId!=null){
		...
		this.realDays(this.performReal);
		
		calPerform = this.cal;
		calReal = this.real; //가동율에 따른 실제 수행일
		yearDays = this.yearDays;
		
		var position = '';
		if(JSV.browser.msieUnder9) {
			position = '<p style="vertical-align:middle;">';
		} else {
			position = '<p style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);">';
		}
		$('<div>').addClass('performTd').width('7%').css({'position':'absolute','right':'7%','top':'0'}).appendTo(this.$myTd)
				.html(position+Math.round(((this.real/this.yearDays)*100)*10)/10 +'%'+'</p>')
	...
	ProjectYearViewer.prototype.addTodayLine = function(addToday){
		...
		$('<div>').css({'position':'absolute','left':startLeft+'%', 'top':top-3.5+'px'}).
