...
JSV.Block(function () {
	...
	if(enhId){
		if(JSV.getParameter('isDel') != null){
			var json=JSV.loadJSON('/jsl/EnhOwner.SelectProject.json?enhId='+enhId+'&nYear='+nYear+'&isDel=true');
		}else{
			var json=JSV.loadJSON('/jsl/EnhOwner.SelectProject.json?enhId='+enhId+'&nYear='+nYear);
		}
		var jsl=json.array;
 	}
...
function ProjectYearViewer(parent, style){
	...
	if(this.mode=='existData'){
		$('<td>').appendTo($tr).addClass('monTd1')
			.width('7%').text(JSV.getLang('EmployeeType','inner'));
	}
