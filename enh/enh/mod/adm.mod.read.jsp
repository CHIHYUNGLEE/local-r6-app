...
JSV.Block(function () {
	...
	if(<%= isAdmin(moduleParam)%>) {
		...
		rightBtnArr.push({text:<fmt:message key="btn.pub.remove"/>.text,
			  onclick:function() {
					if (confirm('<fmt:message key="pub.045"/>')) {
						t.submitInner('/jsl/EnhAdmin.DoAdminModRemove.jsl?modid=@{modid}',function(value){
							if(parent){
								parent.tabRefresh(model.enhId);
								parent.tab.admRomove(model.seqOrder, model.enhId);
							}
						});
					}
			}});
