...
JSV.Block(function () {
	var template = '<template color="blue" catalog="/enh/mod/catalog.xml.jsp">\
			<header label="<fmt:message key="doc.211"/>"/>\
	...
	apply.onclick = function() {
		if (t.validate('title,content')) {
			...
			t.submitInner('/jsl/EnhModUser.DoUpdateByUser.json',function(value){
				if(parent){
					var enhId = JSV.getParameter("enhid");
					parent.tabSortAdminReload(value, enhId); // 글 수정 후 tabid인 value값을 부모함수로 넘겨준다.
				}
			});
		}
	}
