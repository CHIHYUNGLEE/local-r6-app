...
%>;
	...
	var json = JSV.loadJSON('/jsl/EnhModUser.ModDeleteList.json?enhid=' + JSV.getParameter('id')); // id => enhid
	...
	tab = new TabSortAdmin(document.getElementById('tabDiv'), 
			{
				'readUrl':'/enh/adm.read.jsp',
				'tabUrl':'/enh/mod/adm.mod.read.jsp'
			}
	);
...
function tabSortAdminReload(modid){
	...
	var json = JSV.loadJSON('/jsl/EnhModUser.ModDeleteList.json?enhid=' + JSV.getParameter('id'));
...
function tabRefresh(enhid){
	var json = JSV.loadJSON('/jsl/EnhModUser.ModDeleteList.json?enhid=' + enhid);
	var model = json.array;
 	tab.value = (model && model.length>0)? model : [];
 	tab.reset();
}
