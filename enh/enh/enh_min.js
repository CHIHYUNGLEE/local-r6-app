...
EnhTaskStatusFieldEditor.prototype.add=function(option){var a=$('<a href="javascript:void(0);"></a>').attr('value',option.value).appendTo($('<li>').appendTo(this.ul));$('<span>').addClass('txt').text(option.text).appendTo(a);a.click(this,function(e){var $this=$(this);e.data.value=$this.attr('value');if(e.data.value==TaskItem.COMPLETE){var url=JSV.getContextPath(DateFieldEditor.DIALOG_SRC);var width='250px';var height=JSV.browser.msie6?'245px':'235px';var feature='dialogWidth:'+width+';dialogHeight:'+height+';scroll:no;status:no;resizable:no;';JSV.showModalDialog(url,null,feature,function(time){if(time){e.data.ul.find('li.selected').removeClass('selected');$this.parent().addClass('selected');console.log(time);e.data.doUpdate(time);}});}else{e.data.ul.find('li.selected').removeClass('selected');$this.parent().addClass('selected');e.data.doUpdate(null);}});this.statuses[option.value]=option.text;}
...
EnhTaskStatusFieldEditor.prototype.doUpdate=function(time){var id=this.id||JSV.getParameter(this.style.attribute);let data={id:id,status:this.value};if(time){data.completeDate=time;}
if(this.editable){$.ajax({url:JSV.getModuleUrl(JSV.getContextPath(this.style.action||EnhTaskStatusFieldEditor.action)),type:'POST',dataType:'json',data:data,context:this,async:false,success:function(data,status){if(data.error!=undefined){alert(JSV.getLang('TaskEmpsViewer','permissionDenied'));}else{this.onsuccess(data);}},error:function(xhr){this.onerror();}});}}
