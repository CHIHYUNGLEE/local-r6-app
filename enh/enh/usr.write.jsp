...
JSV.Block(function(){
	var template='<template name="item" color="green" catalog="/enh/catalog.xml.jsp">\
		<header label="<kfmt:message key="enh.008"/>"/>\
	...
	t.setValue('/jsl/EnhUser.PreWrite.json');
	...
	save.onclick = function() {
		if (t.validate('enhName,orgInfo')) {
			var u = '/enh/usr.list.jsp';
			...
			t.submit('/jsl/EnhUser.DoRegister.jsl');
		}
	}
...
var t;
EmpFieldEditor.showPopup = function(value, popupUrl) {
	EmpFieldEditor.value = value;
	var u = popupUrl ? JSV.getContextPath(popupUrl) : JSV.getContextPath('/sys/jsv/usr/EmpFieldEditorDialog.jsp');
	var n = 'empFieldEditor';
	var f = 'width=800,height=780,scrollbars=no,status=no,toolbar=no,menubar=no,location=no,resizable=no';
	window.open(u, n, f);
}
